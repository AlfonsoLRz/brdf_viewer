...
Model3D::ModelComponent* Primitives::getSemisphereModelComp(const unsigned sectors, const unsigned stacks, const float radius)
{
	Model3D::ModelComponent* modelComp = new Model3D::ModelComponent(nullptr);
	std::vector<Model3D::VertexGPUData> geometry;
	...
	float sectorStep = M_PI / sectors;
	float stackStep = M_PI / stacks;
	float sectorAngle, stackAngle;

	for (int i = 0; i <= stacks; ++i)
	{
		stackAngle = M_PI / 2.0f - i * stackStep;			// Starting from pi/2 to 0
		xy = radius * cosf(stackAngle);						// r * cos(u)
		position.z = radius * sinf(stackAngle);				// r * sin(u)
